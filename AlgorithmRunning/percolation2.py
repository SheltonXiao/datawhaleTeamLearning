...
class Percolation(object):
    ...
    def show_percolates(self):
        ...
        for i in range(1,self.n):
            ...
            show[i,:] = np.floor((self.data[i,:] + show[i-1,:])/2)
            cu = np.where(show[i,:]==1)[0].tolist()

            show[i,:] = self._sub_show_percolates(cu,self.data[i,:],show[i,:])
            
        return show + self.data
            
    def _sub_show_percolates(self,cu,data,show):
        if len(cu) == 0:
            return show
        ...
        show[cunew] = 1
        show = self._sub_show_percolates(cunew,data,show)
        return show
